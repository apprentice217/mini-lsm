...
namespace minidb
{
    ...
    Status DBImpl::Init()
    {
        ...
        last_sequence_ = versions_->last_sequence();
        ...
        versions_->set_last_sequence(last_sequence_);
    ...
    Status DBImpl::Write(const WriteOptions &options, WriteBatch *updates)
    {
        ...
        {
            s = Status::IOError("WAL is not available");
        ...
        if (!s.ok())
        {
            // WAL 可能已经部分写入，或写入完成但同步失败。
            // 保存首次错误，后续写入由 MakeRoomForWrite 拒绝。
            if (background_error_.ok())
            {
                background_error_ = s;
            }
    ...
    {
        // 约定：调用方必须持有 mutex_。
        if (bg_compaction_scheduled_ || shutting_down_ || !background_error_.ok())
            return;
    ...
    {
        if (files_per_level == nullptr || bytes_per_level == nullptr)
            return;
        std::lock_guard<std::mutex> lock(mutex_);
